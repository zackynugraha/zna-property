...
app.get('/api/dashboard',asyncHandler(async(req,res)=>{
  ...
  const gross=Number(income.value),oneOff=Number(expense.value),rentCost=Number(masterRent.value),totalCosts=rentCost+oneOff;
  res.json({income:gross,expense:oneOff,masterRent:rentCost,totalCosts,profit:gross-totalCosts,units:Number(units.value),occupied:Number(active.value),occupancy:Number(units.value)?Number(active.value)/Number(units.value)*100:0,arrears:Number(arrears.value),monthly,byApartment});
}));
...
app.get('/api/reports/monthly',asyncHandler(async(req,res)=>{
  ...
  res.json(rows.map(x=>{const income=Number(x.income),rent=Number(x.master_rent),oneOff=Number(x.one_off_expense);return {...x,income,master_rent:rent,one_off_expense:oneOff,total_cost:rent+oneOff,net_profit:income-rent-oneOff}}));
}));
